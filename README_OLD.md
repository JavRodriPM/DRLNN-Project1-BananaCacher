...
Please ve aware that you can select between DQN and DoubleDQN and between the Learning Rate optimization in the agent initialization via the following variables: 
- doubledqn: Set it to true to use a doubleDQN or to false to use a "simple" DQN
- optimizeLR: Set it to true for the learning process to optimize the learning rate periodically or to false for the standard behaviour.

To watch an allready trained smart agent, you can skip section 4. (Train the agent) and goes directly to section 5. where the architecture, weight and bias of a previously training agent are loaded. 
...
You will need to install a standard python package as anaconda along with python pip package administrator in your computer, after this you can use  use the `requirements.txt` file (in the following(https://github.com/JavRodriPM/DDRLN-Project1-BananaCacher/) folder) to install the required dependencies via `pip` 
